...
<br/><br/><br/>
<div>
<form name="customer" action="managerBookSlot.jsp" method="POST" onSubmit="validateselect();"/>
...
<b>BOOK VEHICLE SLOT PAGE</b></font><br/>
...
<%
	try {
		...
		System.out.println("SELECT * FROM REG_VEHICLE WHERE DISTID=(SELECT DISTID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND MANDALID=(SELECT MANDALID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND REG_NO='0' AND REG_VEHICLE.ID NOT IN (SELECT REG_ID FROM SLOT WHERE FLAG=0)");
		ResultSet rs = stmt.executeQuery("SELECT * FROM REG_VEHICLE WHERE DISTID=(SELECT DISTID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND MANDALID=(SELECT MANDALID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND REG_NO='0' AND REG_VEHICLE.ID NOT IN (SELECT REG_ID FROM SLOT WHERE FLAG=0)");
%>
<tr>
