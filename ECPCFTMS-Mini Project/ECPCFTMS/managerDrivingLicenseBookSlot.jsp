...
<br/><br/><br/>
<div>
<form name="customer" action="managerDrivingLicenseDetailsSlot.jsp" method="POST"/>
...
<b>BOOK DRIVING LICENSE SLOT PAGE</b></font><br/>
...
<%
	try {
		...
		ResultSet rs = stmt.executeQuery("SELECT DRIVING_LICENSE.*, (SELECT USERID FROM CUSTOMER WHERE CUST_ID=DRIVING_LICENSE.CUST_ID) AS USERID FROM DRIVING_LICENSE WHERE DISTID=(SELECT DISTID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND MANDALID=(SELECT MANDALID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND DLID='0' AND STATUS='OPEN'");
%>
<tr>
<td align="left">USERID/NAME</td>
<td width="2">&nbsp;</td>
<td width="10">
<SELECT name="ID">
<OPTION VALUE="0">---SELECT---</OPTION>
<%while(rs.next()) { %>
<OPTION value="<%=rs.getInt("ID")%>"><%=rs.getString("USERID")+"-"+rs.getString("NAME")%></OPTION>
<%} %>
</SELECT>
</td>
</tr>
