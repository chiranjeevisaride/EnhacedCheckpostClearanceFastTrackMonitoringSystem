...
function submitForMandal() {
	document.customer.action='managerDrivingLicenseTransfer.jsp';
...
<br/><br/><br/>
<div>
<form name="customer" action="managerDrivingLicenseTransferAction.jsp" method="POST"/>
...
<b>DRIVING LICENSE TRANSFER PAGE</b></font><br/>
...
<%
	try {
		...
		ResultSet rs2 = stmt1.executeQuery("SELECT DRIVING_LICENSE.*,(SELECT USERID FROM CUSTOMER WHERE CUSTOMER.CUST_ID=DRIVING_LICENSE.CUST_ID) AS USERID FROM DRIVING_LICENSE WHERE DISTID=(SELECT DISTID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND MANDALID=(SELECT MANDALID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND DLID='0' AND DRIVING_LICENSE.ID IN (SELECT DRIVE_ID FROM DRIVING_SLOT WHERE STATUS='OPEN')");
%>
<tr>
<td align="left">USER ID/NAME</td>
<td width="2">&nbsp;</td>
<td width="10">
<SELECT name="ID">
<OPTION VALUE="0">---SELECT---</OPTION>
<%while(rs2.next()) { %>
<OPTION <%=(request.getAttribute("ID")!=null && ((String)request.getAttribute("ID")).equalsIgnoreCase(rs2.getString("ID")))?" SELECTED ":"" %> value="<%=rs2.getInt("ID")%>"><%=rs2.getString("USERID")+"-"+rs2.getString("NAME")%></OPTION>
<%} %>
</SELECT>
</td>
</tr>
