...
<br/><br/><br/>
<div>
<form name="customer" action="managerIssueNocSelect.jsp" method="POST"/>
...
<%
	try {
		...
		ResultSet rs = stmt.executeQuery("SELECT * FROM REG_VEHICLE WHERE ID IN (SELECT REG_ID FROM APPLYNOC WHERE STATUS='OPEN') AND DISTID=(SELECT DISTID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND MANDALID=(SELECT MANDALID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+")");
%>
<tr>
