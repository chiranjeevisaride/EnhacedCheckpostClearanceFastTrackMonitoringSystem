  ...
  if(session.getAttribute("ID")==null || session.getAttribute("USERID")==null) {
	...
	try {
		...
		ResultSet rs = stmt12.executeQuery();
		 sql = "SELECT COUNT(*) FROM APPLYFITNESS WHERE STATUS='OPEN' AND REG_ID=?";
		...
		if(rs.next() && rs.getInt(1)!=0) {
			request.setAttribute("response", "FITNESS Application Is Rejected Due To Vehicle Not Registered.");
			request.getRequestDispatcher("customerSelectVehicleFitness.jsp").forward(request,response);
		} if(rs3.next() && rs3.getInt(1)!=0) {
			request.setAttribute("response", "FITNESS Application Is Already Applied.");
			request.getRequestDispatcher("customerSelectVehicleFitness.jsp").forward(request,response);
		} else {
		sql = "INSERT INTO APPLYFITNESS(REG_ID,CUST_ID) VALUES(?,?)";
			stmt = con.prepareStatement(sql);
			stmt.setString(1, request.getParameter("ID"));
			stmt.setString(2, (String)session.getAttribute("ID"));		
			stmt.executeUpdate();
			request.setAttribute("response", "FITNESS Application Is Accepted.");
			request.getRequestDispatcher("customerSelectVehicleFitness.jsp").forward(request,response);
		}
			stmt.close();
		rs.close();
