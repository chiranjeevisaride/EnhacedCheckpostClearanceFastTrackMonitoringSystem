...
<br/><br/><br/>
<div>
<form name="customer" action="managerIssuePermitSelect.jsp" method="POST"/>
...
<b>MANAGER ISSUE VEHICLE PERMIT PAGE</b></font><br/>
...
<%
	try {
		...
		ResultSet rs = stmt.executeQuery("SELECT * FROM REG_VEHICLE WHERE ID IN (SELECT REG_ID FROM NATIONAL_PERMIT WHERE STATUS='OPEN') AND DISTID=(SELECT DISTID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND MANDALID=(SELECT MANDALID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+")");
%>
<tr>
