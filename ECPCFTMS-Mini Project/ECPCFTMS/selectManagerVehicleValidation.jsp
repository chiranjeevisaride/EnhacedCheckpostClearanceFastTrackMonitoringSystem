...
<br/><br/><br/>
<div>
<form name="customer" action="managerVehicleValidation.jsp" method="POST"/>
...
<b>VEHICLE VALIDATION PAGE</b></font><br/>
...
<%
	try {
		...
		ResultSet rs = stmt.executeQuery("SELECT * FROM REG_VEHICLE WHERE DISTID=(SELECT DISTID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND MANDALID=(SELECT MANDALID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND REG_NO='0' AND REG_VEHICLE.ID IN (SELECT REG_ID FROM SLOT WHERE FLAG=0)");
%>
<tr>
