...
<br/><br/><br/>
<div>
<form name="customer" action="customerViewNocAction.jsp" method="POST"/>
...
if(request.getAttribute("STATUS")==null) {
		...
		ResultSet rs = stmt.executeQuery("SELECT REG_VEHICLE.*, (SELECT MAX(ID) FROM APPLYNOC WHERE STATUS!='OPEN'  AND REG_ID=REG_VEHICLE.ID) AS NOCID FROM REG_VEHICLE WHERE cust_id="+session.getAttribute("ID"));
%>
<tr>
...
<%while(rs.next()) { %>
<OPTION value="<%=rs.getInt("NOCID")%>"><%=rs.getString("ENG_NO")+"-"+rs.getString("CHASIS_NO")+"-"+rs.getString("BIKE_NAME")%></OPTION>
<%} %>
</SELECT>
</td>
</tr>
<tr>
<td colspan="3" align="center">
<input type="submit" value="VIEW NOC">
</td>
</tr>
...
Vehicle TAX Paid Status     : <%=request.getAttribute("TAX_PAID")%></br>
Vehicle Any Case Status     : <%=request.getAttribute("ANY_CASE")%></br>
Vehicle Case Reason         : <%=request.getAttribute("CASE_DETAILS")%></br>
</td>
</tr>
<%} %>
