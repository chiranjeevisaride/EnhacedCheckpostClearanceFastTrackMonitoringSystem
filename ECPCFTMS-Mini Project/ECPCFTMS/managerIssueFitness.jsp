...
<br/><br/><br/>
<div>
<form name="customer" action="managerIssueFitnessSelect.jsp" method="POST"/>
...
<%
	try {
		...
		ResultSet rs = stmt.executeQuery("SELECT * FROM REG_VEHICLE WHERE ID IN (SELECT REG_ID FROM APPLYFITNESS WHERE STATUS='OPEN') AND DISTID=(SELECT DISTID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+") AND MANDALID=(SELECT MANDALID FROM EMPLOYEE WHERE ID="+session.getAttribute("ID")+")");
%>
<tr>
